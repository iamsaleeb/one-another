...
beforeEach(() => {
  ...
  mockHandleUpload.mockResolvedValue({ url: 'https://example.public.blob.vercel-storage.com/img.jpg' })
})
...
describe('POST /api/upload', () => {
  ...
  describe('token options (via onBeforeGenerateToken)', () => {
    it('returns allowedContentTypes covering common image formats', async () => {
      mockAuth.mockResolvedValue({ user: { id: 'user-1', role: 'MEMBER' } })
      let capturedOpts: Awaited<ReturnType<Parameters<typeof handleUpload>[0]['onBeforeGenerateToken']>> | undefined
      ...
      expect(capturedOpts?.allowedContentTypes).toEqual(
        expect.arrayContaining(['image/jpeg', 'image/png', 'image/webp', 'image/gif', 'image/avif'])
      )
    })
    ...
    it('includes userId and variant in tokenPayload', async () => {
      mockAuth.mockResolvedValue({ user: { id: 'user-42', role: 'ORGANISER' } })
      let capturedOpts: Awaited<ReturnType<Parameters<typeof handleUpload>[0]['onBeforeGenerateToken']>> | undefined
  ...
  describe('success path', () => {
    it('returns 200 with handleUpload response', async () => {
      const mockResponse = { url: 'https://test.public.blob.vercel-storage.com/img.jpg', token: 'tok' }
