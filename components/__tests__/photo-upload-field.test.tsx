...
describe("PhotoUploadField", () => {
  ...
  describe("idle state", () => {
    it("renders upload area for cover variant", () => {
      ...
      expect(
        screen.getByRole("button", { name: "Add cover photo" })
      ).toBeInTheDocument();
    });
  ...
  describe("preview state", () => {
    it("renders cover photo preview when value provided", () => {
      ...
      expect(
        screen.getByRole("img", { name: "Cover photo" })
      ).toBeInTheDocument();
    });
    ...
    it("does not render upload area when value provided", () => {
      ...
      expect(
        screen.queryByRole("button", { name: "Add cover photo" })
  ...
  describe("file validation", () => {
    ...
    it("shows error for file over 4MB and does not upload", async () => {
      ...
      fireEvent.change(input);
      expect(
        await screen.findByText("Image must be 4MB or less.")
      ).toBeInTheDocument();
      expect(mockUpload).not.toHaveBeenCalled();
    });
  });
