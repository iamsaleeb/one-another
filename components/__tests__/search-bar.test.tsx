...
describe("SearchBar — initial render", () => {
  it("renders the search input", () => {
    setup();
    expect(
      screen.getByPlaceholderText(/search events & churches/i)
    ).toBeInTheDocument();
  });
...
describe("SearchBar — active filter pills", () => {
  ...
  it("removes the when pill and navigates on its X button click", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /remove tomorrow filter/i })
    );
  ...
  it("removes the category pill and navigates on its X button click", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /remove outreach filter/i })
    );
    expect(mockPush).toHaveBeenCalledWith("/");
  });
  ...
  it("removes the type pill and navigates on its X button click", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /remove churches only filter/i })
    );
    expect(mockPush).toHaveBeenCalledWith("/");
  });
});
...
describe("SearchBar — dropdown", () => {
  ...
  it("shows all category options in dropdown", async () => {
    ...
    await userEvent.click(screen.getByPlaceholderText(/search events/i));
    expect(
      screen.getByRole("button", { name: "Youth Meeting" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Bible Study" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Servants Meeting" })
    ).toBeInTheDocument();
