...
const mockPrefFindUnique = prisma.notificationPreference
...
describe("scheduleEventReminderNotification", () => {
  ...
  it("uses hoursBeforeEvent from stored preference", async () => {
    ...
    expect(mockUpsert).toHaveBeenCalledWith(
      expect.objectContaining({
        create: expect.objectContaining({
          body: "Sunday Service starts in 4 hours",
        }),
      })
    );
  });
  ...
  it("skips scheduling when scheduledFor is in the past", async () => {
    ...
    const pastEvent = {
      id: "ev-2",
      title: "Past Event",
      datetime: new Date(Date.now() + 1_000),
    };
  ...
  it('uses singular "1 hour" body when hoursBeforeEvent is 1', async () => {
    ...
    expect(mockUpsert).toHaveBeenCalledWith(
      expect.objectContaining({
        create: expect.objectContaining({
          body: "Sunday Service starts in 1 hour",
        }),
      })
    );
  });
...
describe("updateUserReminderSchedule", () => {
  it("updates body and scheduledFor for each future pending reminder", async () => {
    const futureEventDatetime = new Date(
    ...
    mockFindMany.mockResolvedValue([
      {
        id: "n-1",
        data: {
          eventTitle: "Morning Prayer",
          eventDatetime: futureEventDatetime,
        },
      },
    ]);
    mockUpdate.mockResolvedValue({});
    ...
    expect(mockUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "n-1" },
        data: expect.objectContaining({
          body: "Morning Prayer starts in 3 hours",
        }),
      })
    );
  });
  ...
  it("skips reminders where new scheduledFor would be in the past", async () => {
    const nearFutureDatetime = new Date(
      Date.now() + 1 * 3_600_000
    ).toISOString(); // 1h from now
    mockFindMany.mockResolvedValue([
      {
        id: "n-1",
        data: { eventTitle: "Soon Event", eventDatetime: nearFutureDatetime },
      },
    ]);
