...
describe("PushNotificationProvider", () => {
  ...
  it("uses token.value directly on Android (no FCM.getToken call)", async () => {
    const { FCM } = jest.requireMock("@capacitor-community/fcm") as {
      FCM: { getToken: jest.Mock };
    };
    Capacitor.isNativePlatform.mockReturnValue(true);
    ...
    const registrationCall = mockAddListener.mock.calls.find(
      ([event]) => event === "registration"
    );
    const callback = registrationCall?.[1] as (t: {
      value: string;
    }) => Promise<void>;
  ...
  it("shows a toast when a foreground notification is received", async () => {
    ...
    const callback = receivedCall?.[1] as (n: {
      title: string;
      body: string;
    }) => void;
