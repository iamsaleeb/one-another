...
import { assignPlatformRoleAction, removePlatformRoleAction } from '../platform-roles'
import { auth } from '@/auth'
import { sessionToClaims } from '@/domains/roles/lib/session'
import { upsertPlatformRole, removePlatformRole } from '@/domains/roles/dal/platform-roles'
...
describe('assignPlatformRoleAction', () => {
  ...
  it('returns error when unauthenticated', async () => {
    ...
    const result = await assignPlatformRoleAction({ userId: 'u1', role: 'PLATFORM_ADMIN' })
  ...
  it('returns error when caller is not platform admin', async () => {
    ...
    mockSessionToClaims.mockReturnValue({ isPlatformAdmin: false, churchMemberships: [] })
    const result = await assignPlatformRoleAction({ userId: 'u1', role: 'PLATFORM_ADMIN' })
    expect(result).toEqual({ error: 'Unauthorised.' })
  ...
  it('assigns platform role when caller is platform admin', async () => {
    ...
    mockSessionToClaims.mockReturnValue({ isPlatformAdmin: true, churchMemberships: [] })
