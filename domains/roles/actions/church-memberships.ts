...
import { upsertChurchMembership, removeChurchMembership } from '../dal/church-memberships'
import { AssignChurchRoleSchema, RemoveChurchMembershipSchema } from '../validations/roles'
...
export async function assignChurchRoleAction(input: unknown): Promise<RoleActionState> {
  ...
  if (!churchPolicy.canManageMembers(claims, churchId)) return { error: 'Unauthorised.' }

  await upsertChurchMembership(userId, churchId, role, session!.user.id)
  return { success: 'Role assigned.' }
}

export async function removeChurchMembershipAction(input: unknown): Promise<RoleActionState> {
  const session = await auth()
  const claims = sessionToClaims(session)
  if (!claims) return { error: 'Unauthorised.' }
