...
import { AssignEventRoleSchema, RemoveEventStaffSchema } from '../validations/roles'
...
export async function assignEventRoleAction(input: unknown): Promise<RoleActionState> {
  ...
  if (!eventPolicy.canManageStaff(claims, eventId, churchId)) return { error: 'Unauthorised.' }
