...
export async function assignPlatformRoleAction(input: unknown): Promise<RoleActionState> {
...
export async function removePlatformRoleAction(input: unknown): Promise<RoleActionState> {
  const session = await auth()
  const claims = sessionToClaims(session)
  if (!claims) return { error: 'Unauthorised.' }
  if (!claims.isPlatformAdmin) return { error: 'Unauthorised.' }
