...
export function can(
  ...
  if (context.scope === 'CHURCH') {
    ...
    return (CHURCH_ROLE_CAPABILITIES[membership.role] as string[]).includes(capability)
  }
  ...
  if (context.scope === 'EVENT') {
    ...
    if (membership && (CHURCH_ROLE_CAPABILITIES[membership.role] as string[]).includes(capability)) {
      return true
    }
    return false
  }
