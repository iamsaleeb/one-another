...
describe('syncEventQuestions', () => {
  it('deletes existing questions and creates new ones', async () => {
    ...
    expect(mockQuestionDeleteMany).toHaveBeenCalledWith({ where: { eventId: 'evt-1' } })
  ...
  it('silently skips sync when responses already exist (questions locked)', async () => {
    ...
    await expect(syncEventQuestions('evt-1', [baseQuestion], 'user-1')).resolves.toBeUndefined()
  ...
  it('upserts unique (label, type) pairs into the library', async () => {
    const questions = [
      ...
      { ...baseQuestion, id: 'q-3', label: 'Age?', type: QuestionType.SHORT_TEXT },
    ]
    ...
    expect(mockLibraryUpsert).toHaveBeenCalledWith(
      expect.objectContaining({ create: expect.objectContaining({ label: 'Name?' }) })
    )
    expect(mockLibraryUpsert).toHaveBeenCalledWith(
      expect.objectContaining({ create: expect.objectContaining({ label: 'Age?' }) })
    )
  })
  ...
  it('assigns sequential order values to questions', async () => {
    ...
    const created = mockQuestionCreateMany.mock.calls[0][0].data as Array<{ order: number; label: string }>
