...
describe('saveResponses', () => {
  it('upserts valid responses', async () => {
    ...
    expect(mockResponseUpsert).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { eventAttendeeId_questionId: { eventAttendeeId: 'attendee-1', questionId: 'q-1' } },
  ...
  it('accepts file URLs that start with https://', async () => {
    const responses = [
      { questionId: 'q-1', answer: null, fileUrl: 'https://blob.vercel.com/file.pdf' },
    ]
    ...
    expect(mockResponseUpsert).toHaveBeenCalledWith(
      expect.objectContaining({
        create: expect.objectContaining({ fileUrl: 'https://blob.vercel.com/file.pdf' }),
      })
    )
  })
  ...
  it('throws when a required question has no answer', async () => {
    ...
    const responses = [{ questionId: 'q-2', answer: 'optional', fileUrl: null }]

    await expect(saveResponses('attendee-1', responses, 'evt-1')).rejects.toThrow(
  ...
  it('accepts a required question answered with a file URL', async () => {
    ...
    const responses = [
      { questionId: 'q-1', answer: null, fileUrl: 'https://blob.vercel.com/doc.pdf' },
    ]

    await expect(saveResponses('attendee-1', responses, 'evt-1')).resolves.toBeUndefined()
    expect(mockTransaction).toHaveBeenCalled()
  })
