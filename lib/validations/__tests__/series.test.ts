...
describe("createSeriesSchema", () => {
  ...
  it("accepts all valid cadence values", () => {
    for (const cadence of [
      ...
      expect(
        createSeriesSchema.safeParse({ ...validInput, cadence }).success
      ).toBe(true);
    }
  });
  ...
  it("requires a churchId", () => {
    ...
    if (!result.success)
      expect(result.error.flatten().fieldErrors.churchId).toBeDefined();
  });
  ...
  it("rejects an invalid cadence", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success)
      expect(result.error.flatten().fieldErrors.cadence).toBeDefined();
  });
